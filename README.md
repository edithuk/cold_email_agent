# 🌊 DripFlow (Premium CRM Outreach Suite)

DripFlow is a sleek, state-of-the-art cold email outreach CRM web application designed to run highly personalized cold outreach and follow-up drip campaigns. Built with security, deliverability, and developer ergonomics in mind, it allows users to manage contacts, draft high-converting templates, run pre-flight spam checks, and schedule automated multi-stage follow-ups.
...
- **Aggregate Outreach Metrics**: An elegant dashboard metrics bar tracking:
...
4. **📈 Step 4: Monitor & Send Control**:
   - **Pause/Resume & Stop Buttons**: Instantly pause, resume, or abort campaigns mid-execution directly from the dashboard.
   - **Real-Time Rich Email Preview**: View the compiled subject and body for the current recipient side-by-side with active logs.
   - Real-time progress bar, auto-scrolling log console, active recipient table, and background follow-up job dispatch schedule lists.
...
## 🛠️ Architecture & Tech Stack (How it does it)
...
- **Backend (API Server)**: Node.js Express server, `nodemailer` (SMTP transport).
...
Sensitive SMTP App Passwords are encrypted on the client side using **AES-256-GCM** before sending the payload to Firestore. The background dispatcher securely derives the decryption key matching the logged-in user profile dynamically to dispatch drip schedules, ensuring a zero-knowledge standard.
...
## 🚀 Local Setup & Configuration (How to setup)
...
1. Configure Authentication, Firestore Database, and Web App inside your **[Firebase Console](https://console.firebase.google.com)**.
2. Initialize or bind your local repository to your Firebase project:
   ```bash
   npx firebase use --add
   ```
3. Set up the local frontend configuration. Create `/frontend/.env.local` and add your keys:
   ```env
...
### ⚡ 4. Background Job Deployment & Index Setup
To handle background follow-up emails securely when the browser is closed:

1. **Synchronize Encryption Secret**: Save your encryption salt to Google Cloud Secret Manager so the backend can decrypt your local SMTP key:
   ```bash
   npx firebase functions:secrets:set ENCRYPTION_SALT
   ```
...
- **Local Dev Proxy**: Proxy configuration in `vite.config.js` routes local `/api/*` traffic automatically to `http://localhost:3001`.

---

## 📑 How to Run an Outreach Campaign (User Manual)
...
6. **Dispatch & Track**:
   ...
   - Use the **Pause/Resume** and **Stop** controls to manage dispatch flow dynamically.
